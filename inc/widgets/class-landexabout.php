<?php
/**
 * Landex class-landex-short-about.php widget class file
 *
 * @package Landex
 */
 ...
 */
class LandexAbout extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'landex_about', // Base ID
			esc_html__( 'Landex About', 'landex' ), // Name
			array( 'description' => esc_html__( 'Display Footer Short About Info', 'landex' ) ) // Args
		);
	...
	public function widget( $args, $instance ) {
		...
		?>
		<div class="landex-footer-about-wrap">
			...
			<p class="text-landex-manatee"><p><?php echo wp_kses_post( $description ); ?></p>
			<?php endif ?>
			<div class="landex-social-icon">
	...
	public function form( $instance ) {
		...
		<div id="fa_all_markup_wrap" style="display: none;">
			<div class="fa-all-thing-wrapper">
				<div class="themify-icon-search">
					<input type="text" class="themify_search" placeholder="<?php echo esc_attr_e('Search themify icon', 'landex'); ?>">
				</div>
				<div class="themify-icon-list-wrap">
					<ul>
					<?php
					if ( function_exists( 'landex_get_social_icons' ) ) :
						$themify_icons = landex_get_social_icons();
				...
				<button class="fa_tb_close button-primary"><?php esc_html_e( 'Insert Icon', 'landex' ); ?></button>
			</div>
		</div>
		<p>
			<div class="single-widget-uploader">
				<label for="<?php echo esc_attr( $this->get_field_id( 'thumb' ) ); ?>">
					...
					<button class="sl-widget-up-btn button" data-url="<?php echo esc_url( get_site_url() ); ?>"><?php echo esc_html__( 'Upload image', 'landex' ); ?></button>
					<?php $thumnbnail = ! empty( $thumb ) ? esc_attr( $thumb ) : ''; ?>
					<button <?php echo ( ! empty( $thumnbnail ) ) ? 'style="display: inline-block;"' : ''; ?> class="sl-widget-media-remove button" data-url="<?php echo esc_url( get_site_url() ); ?>"><?php echo esc_html__( 'Remove image', 'landex' ); ?></button>
					<br><br>
				<?php
				if ( ! empty( $thumnbnail ) ) :
					?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description:', 'landex' ); ?></label>
		...
		<div class="landex-repeater-info-wrapper" data-id-name="landex_about" data-field-id="social_links" data-placeholder="<?php esc_attr_e("Social Page Url", "landex"); ?>">
			...
			<div class="single-repeater-field-wrap">
				...
				<div class="single-repater-content">
					<div class="single-widget-icon-wrapper">
						<label for="<?php echo esc_attr( $this->get_field_id( 'social_links' ) . '[' . esc_attr( $index ) . '][thumb-id]' ); ?>">
							...
							if ( ! empty( $item['thumb'] ) ) {
								...
								esc_html_e( 'Select an Icon', 'landex' ); }
							?>
							</span></a>
					...
					<input class="sl-repater-title-in" type="text" placeholder="<?php esc_attr_e('Social Page Url', 'landex'); ?>" name="<?php echo esc_attr( $this->get_field_name( 'social_links' ) . '[' . esc_attr( $index ) . '][title]' ); ?>" value="<?php echo ( ! empty( $item['title'] ) ) ? esc_attr( $item['title'] ) : ''; ?>">
			...
			<a class="button landex-widget-add-item" data-widget-type="contact_info"><?php esc_html_e( 'Add Social Link', 'landex' ); ?></a>
		</div>
		<?php
	}
...
if ( ! function_exists( 'landex__about_widget_init' ) ) {
	function landex__about_widget_init() {
		register_widget( 'LandexAbout' );
	}
}
add_action( 'widgets_init', 'landex__about_widget_init' );
